...
def replace_site_assets_url(text: str) -> str:
    ...
    return template.render(
        url=options["url"],
        assets_url=urllib.parse.urljoin(options["url"] + "/",
                                        options["assets_dir"]),
    )
...
def replace_static_url(text: str) -> str:
    template = jinja2.Environment(loader=jinja2.BaseLoader()).from_string(text)
    return template.render(
        static_url=urllib.parse.urljoin(options["url"] + "/", "static"))


# all post-processing goes here
def post_process(html: str) -> str:
    soup = bs4.BeautifulSoup(html, "html.parser")
    # add target-blank for different site
...
class SinglePage(Page):
    ...
    def load(self, navs: list):
        ...
        with open(src_path, "r") as f:
            self.content = markdown2.markdown(replace_site_assets_url(
                f.read()),
                                              extras=markdown_extras)
            self.output = page_template.render(options=options,
                                               navs=navs,
                                               page=self)
...
class CollectionPage(Page):
    ...
    def load(self, navs: list):
        ...
        self.output = collection_template.render(options=options,
                                                 navs=navs,
                                                 page=self,
                                                 posts=self.posts)
        self.output = post_process(self.output)
...
class PostPage(Page):
    ...
    def __init__(self, name: str, title: str, parent: Page):
        ...
        if not parent or not isinstance(parent, CollectionPage):
            raise Exception(
                "post page has no parent or parent is non-collection")
        self.parent = parent
    ...
    def load(self, navs: list):
        src_path = os.path.join(input_dir, self.src)
        with open(src_path, "r") as f:
            content = markdown2.markdown(replace_site_assets_url(f.read()),
                                         extras=markdown_extras + ["metadata"])
...
def build_skeleton(output_dir: str = output_dir):
    ...
    shutil.copytree(os.path.join(template_dir, "static"),
                    os.path.join(output_dir, "static"))
    shutil.copyfile(os.path.join(template_dir, "robots.txt"),
                    os.path.join(output_dir, "robots.txt"))
    shutil.copytree(options["assets_dir"],
